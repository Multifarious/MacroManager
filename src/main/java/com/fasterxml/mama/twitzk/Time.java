...
public enum Time implements Unit<Time> {
  ...
  private final static Pattern PAT = Pattern.compile("(\\d+)\\s*(\\w");
  
  public static Amount<Integer, Time> fromString(String input)
  {
      Matcher m = PAT.matcher(input);
      if (!m.matches()) {
          throw new IllegalArgumentException("Invalid time representation '"+input+"'");
      }
      try {
          int magnitude = Integer.parseInt(m.group(1));
          Time unit = Time.valueOf(m.group(2).toLowerCase());
          return new Amount<Integer, Time>(magnitude, unit, Integer.MAX_VALUE) {
