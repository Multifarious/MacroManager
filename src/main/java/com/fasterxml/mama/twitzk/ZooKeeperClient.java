...
public class ZooKeeperClient {
   ...
   */
  public static ZKCredentials digestCredentials(String username, String password) {
  ...
  final static class SessionState {
    final long sessionId;
    final byte[] sessionPasswd;

    SessionState(long sessionId, byte[] sessionPasswd) {
      this.sessionId = sessionId;
      this.sessionPasswd = sessionPasswd;
    }
  }

  static final Logger LOG = Logger.getLogger(ZooKeeperClient.class.getName());

  static final Amount<Long,Time> WAIT_FOREVER = Amount.of(0L, Time.MILLISECONDS);

  private final int sessionTimeoutMs;
  private final ZKCredentials credentials;
  ...
  final BlockingQueue<WatchedEvent> eventQueue = new LinkedBlockingQueue<WatchedEvent>();
  ...
  public ZooKeeperClient(String sessionTimeout, InetSocketAddress zooKeeperServer,
          InetSocketAddress... zooKeeperServers) {
        this(Time.fromString(sessionTimeout), combine(zooKeeperServer, zooKeeperServers));
      }
  ...
  public ZooKeeperClient(String sessionTimeout,
          Iterable<InetSocketAddress> zooKeeperServers) {
      this(Time.fromString(sessionTimeout), ZKCredentials.NONE, null, zooKeeperServers);
  }
  ...
  public ZooKeeperClient(Amount<Integer, Time> sessionTimeout,
      Iterable<InetSocketAddress> zooKeeperServers) {
    this(sessionTimeout, ZKCredentials.NONE, null, zooKeeperServers);
  }
   ...
   * @param zooKeeperServers any additional servers forming the ZK cluster
   */
  public ZooKeeperClient(Amount<Integer, Time> sessionTimeout, ZKCredentials credentials,
      InetSocketAddress zooKeeperServer, InetSocketAddress... zooKeeperServers) {
    this(sessionTimeout, credentials, null, combine(zooKeeperServer, zooKeeperServers));
  }
  ...
  public ZooKeeperClient(Amount<Integer, Time> sessionTimeout, ZKCredentials credentials,
      Iterable<InetSocketAddress> zooKeeperServers) {
        this(sessionTimeout, credentials, null, zooKeeperServers);
      }
   ...
   */
  public ZooKeeperClient(Amount<Integer, Time> sessionTimeout, ZKCredentials credentials,
      String chrootPath, Iterable<InetSocketAddress> zooKeeperServers) {
    this.sessionTimeoutMs = sessionTimeout.as(Time.MILLISECONDS);
    ...
    this.zooKeeperServers = Joiner.on(',').join(servers).concat((chrootPath == null) ? "" : chrootPath);
  }
   ...
   * @return a set of credentials that can be used to authenticate the zoo keeper client
   */
  public static ZKCredentials credentials(final String scheme, final byte[] authToken) {

    return new ZKCredentials() {
      ...
      @Override public boolean equals(Object o) {
        if (!(o instanceof ZKCredentials)) {
          return false;
        }

        ZKCredentials other = (ZKCredentials) o;
        return _equals(scheme, other.scheme())
                && _equals(authToken, other.authToken());
      }
      ...
      @Override public int hashCode() {
          int hash = (scheme == null) ? 1 : scheme.hashCode();
          if (authToken != null) {
              hash -= authToken.hashCode();
          }
          return hash;
      }
    };
   ...
   * if this client was constructed with {@link ZKCredentials#NONE}.
   *
   * @return {@code true} if this client is configured with non-empty credentials.
   */
  public boolean hasCredentials() {
      final String scheme = credentials.scheme();
      return (scheme != null) && !scheme.isEmpty() && (credentials.authToken() != null);
  }
