...
public class ZooKeeperMap<V> extends ForwardingMap<String, V> {
  ...
  static final Logger LOG = Logger.getLogger(ZooKeeperMap.class.getName());

  private final ZooKeeperClient zkClient;
  private final String nodePath;
  private final Function<byte[], V> deserializer;

  final ConcurrentMap<String, V> localMap;
  ...
  final Object safeToRewatchLock;
  volatile boolean safeToRewatch;
   ...
   *     cluster
   */
  ZooKeeperMap(ZooKeeperClient zkClient,
      String nodePath,
      Function<byte[], V> deserializer,
  ...
  void tryWatchChildren() throws InterruptedException {
  ...
  synchronized void watchChildren()
      throws InterruptedException, KeeperException, ZooKeeperConnectionException {
  ...
  void tryAddChild(final String child) throws InterruptedException {
    backoffHelper.doUntilSuccess(new ExceptionalSupplier<Boolean, InterruptedException>() {
      @Override public Boolean get() throws InterruptedException {
        try {
          addChild(child);
  ...
  void rewatchDataNodes() throws InterruptedException {
    for (String child : keySet()) {
      tryAddChild(child);
    }
  }

  String makePath(final String child) {
    return nodePath + "/" + child;
  }

  void updateChildren(Set<String> zkChildren) throws InterruptedException {
